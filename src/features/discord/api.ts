...
export class DiscordAPI {
  ...
  async getGuildChannelsWithBotToken(
    guildId: string
  ...
  async getGuildAuditLogsWithBotToken(
    guildId: string,
    options: { limit?: number; user_id?: string; action_type?: number } = {}
  ): Promise<DiscordGuildAuditLog> {
    const params = new URLSearchParams();
    if (options.limit) params.append('limit', options.limit.toString());
    if (options.user_id) params.append('user_id', options.user_id);
    if (options.action_type)
      params.append('action_type', options.action_type.toString());
    ...
    return response.json() as Promise<DiscordGuildAuditLog>;
  }
  ...
  async getGuildInvitesWithBotToken(
    ...
    const response = await fetch(
      `http://localhost:4000/v1/guild/invites?serverId=${serverId}&with_counts=${with_counts}`,
      {
        method: 'GET',
      }
    );
  ...
  async deleteGuildInvite(
    ...
    const response = await fetch(
      `http://localhost:4000/v1/guild/invite/delete`,
      {
        method: 'POST',
  ...
  async createGuildInvite(serverId: string): Promise<DiscordInvite> {
    const response = await fetch(
      `http://localhost:4000/v1/guild/invite/create`,
      {
        method: 'POST',
        body: new URLSearchParams({
          serverId: serverId,
