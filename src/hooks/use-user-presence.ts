...
export function useUserPresence(
  serverId: string
): ReturnType<typeof IUseUserPresence>;
export function useUserPresence(
  serverId: string,
  userId: string
): ReturnType<typeof IUseUserPresence>;
      ...
      try {
        ...
        if (userId) {
          ...
          const userRes = await fetch(
            `http://localhost:3000/api/discord/user?userId=${userId}`
          );
          if (!userRes.ok) {
        ...
        const presenceRes = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/v1/presence?serverId=${serverId}&userId=${userIdToUse}`
        );
