...
function MemberRow({
  ...
  serverId: string;
  onAction: (
    type: 'moderate' | 'member' | 'message' | 'roles',
    member: DiscordMember
  ) => void;
  isSelected: boolean;
  ...
  return (
    <div
      className={`group flex items-center px-4 py-3 transition-colors duration-200 border-b border-discord-border/20 hover:bg-discord-darker/40 ${
        isSelected ? 'bg-discord-blurple/20' : ''
      ...
      <div className='flex items-center mr-4'>
        <Checkbox
          ...
          onCheckedChange={checked =>
            onSelect(member.user?.id || member.user.id, !!checked)
          }
      ...
      <div className='flex items-center gap-3 min-w-[200px]'>
        ...
        <div className='flex-1'>
          ...
          {member.user?.discriminator && (
            <div className='text-discord-text text-xs'>
              #{member.user.discriminator}
            </div>
          )}
        </div>
        ...
        <div className='flex items-center gap-1 overflow-x-auto scrollbar-hide'>
          {member.roles && member.roles.length > 0 ? (
            member.roles.map((roleId: string) => {
              ...
              console.log(roleId, roleName, roleColor);
              return (
                <span
                  key={roleId}
            ...
            <span className='text-discord-text text-xs opacity-60'>
              No roles
            </span>
          )}
        </div>
...
export function MembersContent({ serverId }: MembersContentProps) {
  ...
  const [selectedMember, setSelectedMember] = useState<DiscordMember | null>(
    null
  );
  const [ownerId, setOwnerId] = useState<string | null>(null);
  const [modalType, setModalType] = useState<
    null | 'roles' | 'moderate' | 'message'
  >(null);
  ...
  const { membersCache, loadingMembers, rolesCache, fetchRoles, fetchMembers } =
    useDiscordCache();
  ...
  useEffect(() => {
    if (rolesCache) {
      const cachedRoles = Object.values(rolesCache).flat();
  ...
  // Fetch members when search/filter/page changes
  useEffect(() => {
    if (serverId) {
      fetchMembers({
        serverId: serverId,
        pageIdx: page,
        after: afterCursors[page] || '',
        filterRole,
        searchQuery,
        pageSize: PAGE_SIZE,
      });
    }
  }, [afterCursors, fetchMembers, filterRole, page, searchQuery, serverId]);

  // Update members list when cache changes
  useEffect(() => {
    if (membersCache) {
      const cachedMembers = Object.values(membersCache).flat();
  ...
  const handleInvite = async () => {
    ...
    const res = await fetch(
      `/api/discord/guild/invite?serverId=${serverId}&with_counts=1`
    );
  ...
  useEffect(() => {
    if (modalType === 'roles' && serverId) {
      ...
      setRolesError(null);
      try {
        setAllRoles(Object.values(rolesCache).flat());
        setEditRoles(selectedMember?.roles || []);
      } finally {
        setRolesLoading(false);
      }
    }
  }, [modalType, serverId, selectedMember, rolesCache]);
    ...
    <div className='flex-1 overflow-auto bg-discord-dark'>
      ...
      <header className='sticky top-0 z-20 bg-discord-darker/95 backdrop-blur-xl border-b border-discord-border/50 p-8'>
        <div className='flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6'>
          ...
          <p className='text-discord-text text-lg'>
            Manage server members, roles, and permissions
          </p>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant='outline'
                className='discord-button-outline h-12 px-6 bg-transparent'
              >
            ...
            <DropdownMenuContent className='w-64 bg-discord-darker border-discord-border shadow-2xl'>
              ...
              {allRoles.slice(0, 8).map((role: any) => (
                <DropdownMenuItem
                  key={role.id}
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <h2 className='text-3xl font-black text-white mb-2'>
                ACTIVE{' '}
                <span className='text-discord-green glow-text'>MEMBERS</span>
              </h2>
              <p className='text-discord-text text-lg'>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant='ghost'
                        size='sm'
                        className='h-6 w-6 p-0 text-discord-text hover:text-white'
                      >
                        <ArrowUp
                ...
                <div className='w-32 flex items-center gap-2'>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <Calendar
                          className={`w-3 h-3 ${sortKey === 'join' ? 'text-discord-blurple' : ''}`}
                        />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className='w-48 bg-discord-darker border-discord-border'>
                ...
                {members.map((member: DiscordMember, index: number) => (
                  <MemberRow
                    ...
                    onAction={(type: string, memberObj: any) =>
                      openModal(
                        type as 'roles' | 'moderate' | 'message' | null,
                        memberObj
                      )
                    }
        ...
        >
          ...
          >
            <button
              ...
              onClick={() => {
                if (contextMenu.member)
                  openModal('moderate', contextMenu.member);
                setContextMenu({ show: false, x: 0, y: 0, member: null });
              }}
            >
            ...
            <button
              ...
              onClick={() => {
                if (contextMenu.member)
                  openModal('message', contextMenu.member);
                setContextMenu({ show: false, x: 0, y: 0, member: null });
              }}
            >
      ...
      <Dialog open={modalType === 'roles'} onOpenChange={closeModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Manage Roles for {selectedMember?.user?.username}
            </DialogTitle>
          </DialogHeader>
          {rolesLoading ? (
            ...
            <div className='flex flex-col gap-4'>
              <div className='flex flex-wrap gap-2 mt-2 mb-2'>
                ...
                {/* Add role dropdown */}
                <Select onValueChange={handleAddRole} defaultValue=''>
      ...
      <Dialog open={modalType === 'message'} onOpenChange={closeModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Send Message to {selectedMember?.user?.username}
            </DialogTitle>
          </DialogHeader>
          <Textarea
...
function ModerationPanel({
  ...
  const [action, setAction] = useState<'warn' | 'kick' | 'mute' | 'ban'>(
    'warn'
  );
        ...
        {action === 'ban' && (
          <div className='flex items-center gap-2'>
            <Checkbox
              id='deleteMsg'
              checked={deleteMsg}
              onCheckedChange={v => setDeleteMsg(!!v)}
            />
