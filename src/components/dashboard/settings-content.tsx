...
export function SettingsClient({ serverId }: { serverId: string }) {
  ...
  const [initialSettings, setInitialSettings] = useState<typeof form | null>(
    null
  );
  ...
  const hasUnsavedChanges =
    ...
    Object.keys(form).some(
      key =>
        ...
        initialSettings[key as keyof typeof form]
    );
  ...
  const handleRefreshChannels = async () => {
    if (!user) return;
    if (channelRefreshLoading || Date.now() - lastChannelRefresh < 10000)
      return;
                      ...
                      <div className='space-y-3'>
                        ...
                        <div className='relative'>
                          <Input
                            ...
                            value={
                              channels.find(c => c.id === form.logChannelId)
                                ?.name
                          ...
                          <Button
                            ...
                            disabled={
                              channelRefreshLoading ||
                              Date.now() - lastChannelRefresh < 10000
                            }
                            variant={'ghost'}
                          ...
                          >
                            <RefreshCw
                              ...
                              className={
                                channelRefreshLoading ? 'animate-spin' : ''
                              }
                            />
                          </Button>
                          {showChannelDropdown &&
                            (channelQuery.length > 0 || !form.logChannelId) && (
                              <div className='absolute z-20 mt-1 w-full bg-discord-dark border border-white/10 rounded-lg shadow-lg max-h-60 overflow-y-auto'>
                                {channelsLoading ? (
                                  <div className='p-3 text-discord-text'>
                                    Loading channels...
                                  </div>
                                ) : channelsError ? (
                                  ...
                                  channels
                                    .filter(c =>
                                      c.name
                                        .toLowerCase()
                                        .includes(channelQuery.toLowerCase())
                                    )
                                    .slice(0, 10)
                                    .map(channel => (
                                      <div
                        ...
                        {form.logChannelId ? (
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <span>
                                  Please select a log channel to enable
                                  notifications.
                                </span>
                              </TooltipContent>
                            </Tooltip>
          ...
          <CardContent className='space-y-4'>
            <div className='space-y-2'>
              ...
              <Select
                ...
                onValueChange={val =>
                  handleSettingChange('welcomeChannelId', val)
                }
              ...
              <div className='mt-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                  {[
                    {
                      key: '{userName}',
                      desc: 'The username of the new member',
                    },
                    { key: '{userMention}', desc: 'Mentions the new member' },
                    {
                      key: '{userAvatarUrl}',
                      desc: 'Avatar URL of the new member',
                    },
                    { key: '{guildIconUrl}', desc: 'Icon URL of the server' },
                    {
                      key: '{guildBannerUrl}',
                      desc: 'Banner URL of the server',
                    },
                    {
                      key: '{guildMemberCount}',
                      desc: 'Total member count before join',
                    },
                    {
                      key: '{newGuildMemberCount}',
                      desc: 'Total member count after join',
                    },
                    { key: '{serverName}', desc: 'The server name' },
                  ].map(v => (
                    <div
                      key={v.key}
                      className='flex items-center gap-2 bg-discord-dark border border-discord-border rounded px-2 py-1'
                    >
                      <span className='font-mono text-discord-text text-sm select-all'>
                        {v.key}
                      </span>
                      <span className='text-xs text-discord-text flex-1'>
                        {v.desc}
                      </span>
                      <Button
                        type='button'
            ...
            <div className='space-y-2'>
              ...
              <div
                className={`p-3 rounded-lg bg-discord-dark border border-discord-border font-sans font-bold antialiased text-white`}
              >
          ...
          <CardContent className='space-y-6'>
            <div className='flex items-center justify-between'>
              ...
              <Switch
                ...
                onCheckedChange={checked =>
                  handleSettingChange('autoRole', checked)
                }
              />
            </div>
              ...
              <div className='space-y-2'>
                ...
                >
                  ...
                  <SelectContent className='bg-discord-dark border-discord-border text-white max-h-60 overflow-y-auto'>
                    {roles.map(role => (
                      ...
                      >
                        {role.name}
                        {hasHighPermissions(role.permissions) &&
                          ' (⚠️ High Permissions)'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                {highPermRole && (
                  <p className='text-sm text-red-400'>
                    Warning: The selected role has high permissions!
                  </p>
                )}
                <p className='text-sm text-discord-text '>
                  Role that will be automatically assigned
                </p>
              </div>
            )}
            <div className='flex items-center justify-between'>
              ...
              <Switch
                ...
                onCheckedChange={checked =>
                  handleSettingChange('joinNotifications', checked)
                }
            ...
            <div className='flex items-center justify-between'>
              ...
              <Switch
                ...
                onCheckedChange={checked =>
                  handleSettingChange('leaveNotifications', checked)
                }
                disabled={!serverSettings?.logChannelId}
              />
            </div>
