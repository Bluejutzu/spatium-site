...
export function EmbedBuilder({
  ...
  const EmbedPreview = () => (
    <Card className='bg-slate-800 border-l-4 border-blue-500 w-full max-w-md mx-auto'>
      <CardContent className='p-4 space-y-3'>
        ...
        {embed.title && (
          <h3 className='text-blue-400 font-bold text-lg hover:underline cursor-pointer'>
            {embed.title}
          </h3>
        )}
        {embed.description && (
        ...
        {embed.fields && embed.fields.length > 0 && (
          <div className='grid gap-2'>
            {embed.fields.map((field, index) => (
              <div
                key={index}
                className={field.inline ? 'inline-block w-1/3 pr-2' : 'block'}
              >
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent
        className={`bg-slate-800 border-slate-700 text-white p-0 ${
          isMobile
      ...
      >
        <DialogHeader
          className={`border-b border-slate-700 ${isMobile ? 'p-4' : 'p-6'}`}
        >
          <div className='flex items-center justify-between'>
            <DialogTitle
          ...
          {!previewMode ? (
            <>
              ...
              <div className='flex-1 flex flex-col overflow-hidden'>
                ...
                >
                  <TabsList
                    className={`bg-slate-700 w-full ${isMobile ? 'grid grid-cols-2' : 'grid grid-cols-4'}`}
                  >
                    <TabsTrigger
                      value='content'
                    ...
                    <ScrollArea className='h-full'>
                      <TabsContent value='content' className='space-y-4 mt-0'>
                        <Card className='bg-slate-700 border-slate-600 mb-4'>
                          ...
                          <CardContent
                            className={`space-y-4 ${isMobile ? 'p-4' : ''}`}
                          >
                            <div>
                              ...
                              <Input
                                ...
                                value={embed.title || ''}
                                onChange={e =>
                                  updateEmbed({ title: e.target.value })
                                }
                        ...
                        <Card className='bg-slate-700 border-slate-600 mb-4'>
                          <CardHeader className={isMobile ? 'p-4' : ''}>
                            ...
                            <div className='text-sm text-slate-400'>
                              {embed.fields?.length || 0}/25 fields
                            </div>
                          </CardHeader>
                          <CardContent
                            className={`space-y-4 ${isMobile ? 'p-4' : ''}`}
                          >
                            {embed.fields?.map((field, index) => (
                              ...
                              >
                                <CardContent
                                  className={`space-y-3 ${isMobile ? 'p-3' : 'p-4'}`}
                                >
                                  <div className='flex items-center justify-between'>
                                    <Badge
                                      variant='outline'
                      ...
                      <TabsContent value='metadata' className='space-y-4 mt-0'>
                        <Card className='bg-slate-700 border-slate-600 mb-4'>
                          ...
                          <CardContent
                            className={`space-y-6 ${isMobile ? 'p-4' : ''}`}
                          >
        ...
        <div
          className={`flex gap-4 border-t border-slate-700 ${isMobile ? 'flex-col p-4' : 'justify-end p-6'}`}
        >
          <Button
            variant='outline'
            onClick={onClose}
