...
interface RolesChannelCacheState {
...
export const useDiscordCache = create<
  ProfileCacheState &
    MembersCacheState &
    RolesChannelCacheState &
    ChannelsCacheState &
    GlobalCacheState
>((set, get) => ({
  profileCache: {},
  ...
  fetchMembers: async ({
    ...
    try {
      ...
      const res = await fetch(
        `/api/discord/guild/members?${params.toString()}`
      );
      const data = await res.json();
      let filtered = data.members;
      if (filterRole !== 'all') {
        filtered = filtered.filter((m: DiscordMember) =>
          (m.roles || []).includes(filterRole)
        );
      }
      set(state => ({
  ...
  fetchRoles: async (serverId, userId) => {
    ...
    try {
      const res = await fetch(
        `/api/discord/guild/roles?serverId=${serverId}&userId=${userId}`
      );
      ...
      set(state => ({
        ...
        loadingRoles: (() => {
          const updated = new Set(state.loadingRoles);
          updated.delete(key);
          return updated;
        })(),
      }));
  ...
  fetchChannels: async (serverId, userId) => {
    ...
    try {
      const res = await fetch(
        `/api/discord/guild/channels?serverId=${serverId}&userId=${userId}`
      );
      ...
      set(state => ({
        channelsCache: {
          ...state.channelsCache,
          [key]: data.filter((c: DiscordChannel) => c.type === 0),
        },
        loadingChannels: (() => {
          const updated = new Set(state.loadingChannels);
          updated.delete(key);
          return updated;
        })(),
      }));
