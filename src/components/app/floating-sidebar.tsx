...
export function FloatingSidebar() {
    ...
    return (
      <>
        ...
        <Button
          variant="ghost"
          size="icon"
          className="fixed top-4 left-4 z-50 bg-discord-dark/80 backdrop-blur-xl border border-white/10 text-white hover:bg-discord-blurple/20"
          ...
          {mobileMenuOpen && (
            ...
            >
              <motion.div
                ...
                animate={{ x: 0 }}
                exit={{ x: -300 }}
                className="w-80 h-full bg-discord-dark/95 backdrop-blur-xl border-r border-white/10 p-6"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center gap-3 mb-8 mt-12">
                  <div className="p-2 bg-discord-blurple rounded-lg">
                    <Bot className="h-6 w-6 text-white" />
                  </div>
                  <div>
                    <h2 className="text-white font-bold text-lg">DISCORD BOT</h2>
                    <p className="text-discord-text text-sm">Command Center</p>
                  </div>
                </div>
                ...
                <nav className="space-y-2">
                  {navigationItems.map((item) => (
                    ...
                    >
                      ...
                      {item.badge && (
                        <Badge className="ml-auto bg-discord-green text-white text-xs">{item.badge}</Badge>
                      )}
                    </Link>
                  ))}
                </nav>
  ...
  return (
    <TooltipProvider>
      ...
      >
        <div className="bg-discord-dark/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden h-[600px] flex flex-col">
          {/* Header */}
          <div className="p-4 border-b border-white/10">
            <div className="flex items-center gap-3">
              <motion.div
                className="p-2 bg-discord-blurple rounded-lg shadow-glow-blurple"
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <Tooltip key={item.href} delayDuration={0}>
                    ...
                    <Link href={getMobileNavHref(item.href)}>
                      <motion.div
                        className={`flex items-center gap-3 p-3 rounded-lg transition-all cursor-pointer ${isActive
                            ? "bg-discord-blurple text-white shadow-glow-blurple"
                            : "text-discord-text hover:text-white hover:bg-white/5"
                          }`}
                        whileHover={{ x: 2 }}
                        whileTap={{ scale: 0.98 }}
                        ...
                        <AnimatePresence>
                          {isExpanded && (
                            ...
                            >
                              <span className="font-medium">{item.label}</span>
                              {item.badge && (
                                <Badge className="bg-discord-green text-white text-xs">{item.badge}</Badge>
                              )}
                            </motion.div>
                          )}
                        </AnimatePresence>
                  ...
                  {!isExpanded && (
                    <TooltipContent side="right" className="bg-discord-dark border-white/20">
                      <div>
              ...
              >
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-discord-blurple">{totalServers}</div>
                    <div className="text-xs text-discord-text">Servers</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-discord-green">{totalMembers.toLocaleString()}</div>
                    <div className="text-xs text-discord-text">Members</div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {quickActions.map((action, index) => (
                    <Tooltip key={index} delayDuration={0}>
                      <TooltipTrigger asChild>
                        <motion.button
                          className="w-full flex items-center gap-2 p-2 rounded-lg text-discord-text hover:text-white hover:bg-white/5 transition-all"
                          whileHover={{ x: 2 }}
                          whileTap={{ scale: 0.98 }}
                        >
              ...
              <AnimatePresence>
                {isExpanded && (
                  <motion.span
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="text-xs text-discord-text"
                  >
                    System Operational
                  </motion.span>
                )}
